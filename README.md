...
> Import-Organizer - Open source tool by Qyroxen

`cli` `code-organization` `developer-tools` `golang` `imports`

## What is it?

**Import Organizer** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/Qyroxen/Import-Organizer.git
cd Import-Organizer
...
```bash
# Basic usage
./Import-Organizer --path ./target

# With options
./Import-Organizer --path ./target --format json --output report.json

# Verbose mode
./Import-Organizer --path ./target --verbose
```
...
### Basic scan
```bash
./Import-Organizer --path ./my-project
```

### JSON report
```bash
./Import-Organizer --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Import Organizer
  run: ./Import-Organizer --path . --format json --output report.json
```
